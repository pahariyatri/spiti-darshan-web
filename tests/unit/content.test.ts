...
describe('route content', () => {
  ...
  it('rejects broken content', () => {
    const bad = draft({
      status: 'published',
      hero: { ...shimlaToSpiti.hero, media: 'missing-photo' },
    });
    expect(contentProblems([shimlaToSpiti, bad]).join(' ')).toContain(
      'unknown hero image "missing-photo"',
    );
    expect(contentProblems([shimlaToSpiti, draft({ slug: 'Bad Slug' })]).join(' ')).toContain(
      'slug must be',
    );
    expect(contentProblems([shimlaToSpiti, { ...shimlaToSpiti }]).join(' ')).toMatch(
      /duplicate slug|Exactly one/,
    );
    ...
    expect(
      contentProblems([{ ...shimlaToSpiti, days: [day, ...shimlaToSpiti.days.slice(1)] }]).join(
        ' ',
      ),
    ).toContain('mapSegment may only contain');
    const typo = {
      ...shimlaToSpiti.days[0]!,
      stops: [{ name: 'X', type: 'stop' as const, destination: 'nowhere' }],
    };
    expect(
      contentProblems([{ ...shimlaToSpiti, days: [typo, ...shimlaToSpiti.days.slice(1)] }]).join(
        ' ',
      ),
    ).toContain('unknown destination "nowhere"');
  });
});
